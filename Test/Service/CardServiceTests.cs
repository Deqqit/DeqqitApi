...
public class CardServiceTests(CardServiceFixture fixture) : IntegrationTestBase<CardServiceFixture>(fixture)
{
    ...
    public override async Task InitializeAsync()
    {
        ...
        _cardService = new CardService(Context, _mockTemplate.Object, _mockAlgo.Object, _mockTime.Object,
    ...
    {
        Card card = await Context.Cards.FirstAsync(c =>
            c.Note.CreatorId == Fixture.TestCreatorId && c.State != CardState.Suspended);
        ResponseResult<bool> result =
            await _cardService.UpdateCardState(Fixture.TestCreatorId, card.Id, UpdateCardStateRequest.Suspend);
    ...
    public async Task UpdateCardState_Reset_ResetsCard()
    {
        ...
        ResponseResult<bool> result =
            await _cardService.UpdateCardState(Fixture.TestCreatorId, card.Id, UpdateCardStateRequest.Reset);
    ...
    public async Task GetNextStudyCard_PrioritizesLearning()
    {
        ...
        ResponseResult<CardResponse> result =
            await _cardService.GetNextStudyCard(Fixture.TestCreatorId, Fixture.TestDeckId);
    ...
    public async Task SubmitCardReview_ValidRequest_UpdatesCard()
    {
        ...
        Card card = await Context.Cards.FirstAsync(c =>
            c.Note.CreatorId == Fixture.TestCreatorId && c.State == CardState.New);
        ...
        // 2. Act
        ResponseResult<CardResponse> result =
            await _cardService.SubmitCardReview(Fixture.TestCreatorId, card.Id, request);
